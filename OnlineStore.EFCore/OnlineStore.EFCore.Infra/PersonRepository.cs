...
namespace OnlineStore.EFCore.Infra
{
    public class PersonRepository
        : RepositoryBase<Person>, IPersonRepository
    {
        public PersonRepository(OnlineStoreDbContext context) : base(context)
        {
        }

        public PaginationResult<Person> RetrievePersonWithPagination(int page, int itemPerPage, string filter)
        {
            PaginationResult<Person> result = new PaginationResult<Person>();
            if (string.IsNullOrEmpty(filter))
            {
                result.Results = context.Set<Person>().OrderBy(x => x.FirstName)
